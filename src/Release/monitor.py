...
##################################################################
def set_monitor_parameter(
        camera_width, camera_height, thermo_width, thermo_height,
        thermo_mintemp, thermo_maxtemp, thermo_colordepth):
    """iniファイルのパラメータを設定"""
...
def display_camera_thermography_faceframe(
        frame, bicubic, lcd, colors, displayPixelHeight, displayPixelWidth):
    """カメラ画像とサーモグラフィーを表示"""
    ...
    for ix, row in enumerate(bicubic):
        for jx, pixel in enumerate(row):
            pygame.draw.rect(
                lcd, colors[constrain(int(pixel), 0, THERMO_COLORDEPTH - 1)],
                (thermo_offset_x + displayPixelHeight * ix,
                 thermo_offset_y + displayPixelWidth * jx,
...
def display_wait_detect_finish(cap):
    ...
    while True:
        ...
        print(pixels)
        pixels = [mapping(p, THERMO_MINTEMP, THERMO_MAXTEMP,
                          0, THERMO_COLORDEPTH - 1) for p in pixels]
        ...
        if(state == "WAIT"):        # 待機中
            # カメラ画像とサーモグラフィーを表示
            display_camera_thermography_faceframe(
                frame, bicubic, lcd, colors, displayPixelHeight, displayPixelWidth)
