...
def set_sensor_parameter(
        sensor_connected, wait_time, correction_value, detect_start_temperature,
        detect_start_framenum, detect_continue_framenum, fever_temperature):
    """センサで必要なパラメータを設定
...
def measurement_temperature_and_status():
    ...
    if SENSOR_CONNECTED == 1:
        ...
        temperature_array = [27.25, 26.75, 26.75, 27.25, 27.75, 26.75, 27.75, 29.75,
                             27.00, 27.25, 27.25, 26.75, 27.25, 27.50, 29.75, 32.50,
                             27.25, 26.50, 26.75, 27.25, 27.00, 27.50, 27.75, 28.75,
                             26.75, 25.50, 26.50, 23.50, 27.00, 34.75, 27.50, 33.50,
                             26.75, 26.75, 26.75, 27.00, 27.25, 26.75, 27.25, 32.00,
                             26.50, 26.50, 26.50, 27.25, 26.75, 27.00, 28.00, 32.25,
                             26.50, 26.50, 26.50, 27.00, 27.00, 27.25, 28.00, 31.00,
                             26.00, 26.50, 26.00, 26.50, 26.50, 26.75, 26.75, 27.75]
    ...
    temperature_array = list(
        map(lambda x: x + CORRECTION_VALUE if x >= 30.0 else x, temperature_array))
    ...
    def judge_state():
        ...
        # WAIT->DETECT   :
        #   最高温度が[DETECT_START_TEMPERATURE]を連続で[DETECT_START_FRAMENUM]フレーム超える
        # DETECT->FINISH :
        #   DETECT状態になってから最高温度が[DETECT_START_TEMPERATURE]を連続で[DETECT_CONTINUE_FRAMENUM]フレーム超える
        # DETECT->WAIT   :
        #   DETECT状態になってからFINISH条件になる前に最高温度が[DETECT_START_TEMPERATURE]以下になる
        # FINISH->WAIT   :
        #   最高温度が[DETECT_START_TEMPERATURE]以下になる
        global state
        global isfever
